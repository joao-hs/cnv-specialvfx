...
if [ $# -lt 3 ]; then
    echo "Usage: $0 <scene_file> <output_file> <load_balancer_adress> [--texmap <texmap_file>]"
    exit 1
fi
...
# Send the request
curl -sS -X POST "$LOAD_BALANCER_URL/raytracer?scols=400&srows=300&wcols=400&wrows=300&coff=0&roff=0&aa=false" --data @"$_PAYLOAD_FILE" > $_TMP_DIR/$OUTPUT_FILE
